...
public class Test2 {
	...
	By batman = new By.ByCssSelector(".digital-details-title");
	...
	//	wdWait.until(ExpectedConditions.visibilityOfElementLocated(locator));
		...
		int expected = 12;
		Assert.assertEquals("There is "+ elementsCount+" not "+ expected+" as expected", expected, elementsCount);
...
//			int elementsCount = searchResults.size();
//			int expected = 12;
		//	Assert.assertEquals("There is no "+ expected+" than  result ", expected, elementsCount);
			...
			wdWait.until(ExpectedConditions.visibilityOfElementLocated(batman));
			
			String actualString = driver.findElement(batman).getText();
			assertTrue(actualString.contains(searchString));
